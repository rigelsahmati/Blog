$('.loadMoreBtn').on('click', function () {
    var BtnTime = document.querySelector('.loadMoreBtn').dataset.time;
    $.ajax({
        type: 'GET',
        url: "/Blog/LoadPosts",
        dataType: "json",
...
function handleResponse(data) {
    if (data == null) {
        document.querySelector('.loadMoreBtn').classList.add('d-none')
        return;
    };
    document.querySelector('.loadMoreBtn').dataset.time = data.time;
    document.querySelector('.loadMoreBtn').classList.remove('d-none')
    data.posts.forEach((post) => {
        ...
        if (post.owner) {
            ...
            `<button data-actionsId="${post.postId}" class="actionsBtn border border-0 btn">
             ...
             <div id="actionsParent-${post.postId}" class="actionsParent rounded-3 p-2">
                  <a href="Blog/EditPost?postId=${post.postId}" asp-action="EditPost" asp-route-postId="${post.postId}" class="btn text-start"><i class="fa-solid fa-pen me-2"></i> Edit Post</a>
        ...
        var postHtml = `<div class="col-md-7 post position-relative p-3 rounded-3 mt-2 post-${post.postId}">
            ...
            `  <a href="Blog/Post/${post.postId}#coment" class="col btn">
                            ...
                            <button data-ulrlId="${post.postId}" class="copyLinkBtn col btn shareBtn">
